...
def normalizeData(train, test):
    ...
    '''
    train_result = normalize(train, norm='l2', axis=1,
                             copy=True, return_norm=False)
    test_result = normalize(test, norm='l2', axis=1,
                            copy=True, return_norm=False)
    return train_result, test_result


def plot_F_Scores(y_test, y_predict, method, voc_size, classifier):
    '''
    ...
    print("F1: {} (micro), {} (macro), {} (weighted)".format(
        f1_micro, f1_macro, f1_weighted))

    with open("LangDetect/data/F1.csv", "a") as file_object:
        file_object.write(
            f"char_{Method(method).name}, {Classifier(classifier).name}, {f1_micro}, {f1_macro}, {f1_weighted}, {voc_size} ")
        file_object.write('\n')


def plot_Confusion_Matrix(y_test, y_predict, method, voc_size, classifier, color="Blues"):
    '''
    ...
    df_cm = pd.DataFrame(
        confusionMatrix, columns=unqiueLabel, index=unqiueLabel)
    df_cm.index.name = 'Actual'
    df_cm.columns.name = 'Predicted'
    sn.set(font_scale=0.8)  # for label size
    sn.set(rc={'figure.figsize': (15, 15)})
    sn.heatmap(df_cm, cmap=color, annot=True, annot_kws={
               "size": 12}, fmt='g')  # font size
    # plt.show()
    plotName = f'char_Confusion_Matrix_{Method(method).name}_{Classifier(classifier).name}_{voc_size}.png'
    plt.savefig(path + plotName)
    plt.close()


def plotPCA(x_train, x_test, y_test, langs, method, voc_size, classifier):
    '''
    ...
    plotName = f'char_PCA_{Method(method).name}_{Classifier(classifier).name}_{voc_size}.png'
    plt.savefig(path + plotName)
    plt.close()

    with open("LangDetect/data/VarianceExplained.csv", "a") as file_object:
        file_object.write(
            f"char_{Method(method).name}, {Classifier(classifier).name}, {pca.explained_variance_ratio_}, {voc_size}")
        file_object.write('\n')
